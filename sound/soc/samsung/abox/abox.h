...
/**
 * Request abox cpu clock level synchronously with DAI
 * @param[in]	dev		pointer to struct dev which invokes this API
 * @param[in]	data		pointer to abox_data structure
 * @param[in]	dai		DAI which is used as unique handle
 * @param[in]	gear		gear level (cpu clock = aud pll rate / gear)
 * @return	error code if any
 */
static inline int abox_request_cpu_gear_sync_dai(struct device *dev,
		struct abox_data *data, struct snd_soc_dai *dai,
		unsigned int gear)
{
	return abox_request_cpu_gear_sync(dev, data,
			ABOX_CPU_GEAR_DAI | dai->id, gear);
}
